...
const PizzaBlock: React.FC<PizzaBlockProps> = ({ id, imageUrl, title, types, sizes, price }) => {
  const cartItemCount = useAppSelector((state) => state.cart.items.find((item) => item.id === id));
      ...
      <div className="pizza-block__selector">
        <ul>
          {types.map((itemNumber) => (
            <li
              key={itemNumber}
              className={itemNumber === activeType ? 'active' : ''}
              onClick={() => setActiveType(itemNumber)}>
              {typeNames[itemNumber]}
            </li>
          ))}
        </ul>
        <ul>{liElems}</ul>
